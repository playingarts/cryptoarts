...
const Holders: FC<HTMLAttributes<HTMLElement>> = ({ ...props }) => {
  ...
  const stats = {
    ...
    "52+ card decks": holders?.fullDecks?.length ? (
      <span css={{ display: "inline-flex", alignItems: "baseline", height: 40 }}>
        ...
        <span css={(theme) => ({ ...theme.typography["p-m"], color: theme.colors.white50, marginLeft: 10 })}>/150</span>
      </span>
    ) : "...",
    "Full decks": holders?.fullDecksWithJokers ? (
      <span css={{ display: "inline-flex", alignItems: "baseline", height: 40 }}>
        <AnimatedNumber value={holders.fullDecksWithJokers.length} />
        <span css={(theme) => ({ ...theme.typography["p-m"], color: theme.colors.white50, marginLeft: 10 })}>/15</span>
      </span>
    ) : "...",
  };
  ...
  return (
    <div
      ...
      {...props}
    >
      <Text typography="h4">Holders</Text>
      <div css={(theme) => [{ display: "flex", gap: theme.spacing(3), marginTop: theme.spacing(3), [theme.maxMQ.xsm]: { flexWrap: "wrap" } }]}>
        {Object.keys(stats).map((item) => (
          <ScandiBlock
            key={item}
            css={(theme) => [{ paddingTop: 15, display: "block", flex: 1, [theme.maxMQ.xsm]: { flex: "1 1 100%" } }]}
          >
            <Text typography="h3">{stats[item as keyof typeof stats]}</Text>
            <Text typography="p-m">{item}</Text>
          </ScandiBlock>
        ))}
      </div>
      <Charts
