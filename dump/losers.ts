...
import { Loser, MongoCard } from "../source/graphql/schemas/card";
import { Deck } from "../source/graphql/schemas/deck";
import { connect } from "../source/mongoose";

export const cards: Omit<MongoCard, "_id">[] = [
  {
    value: "2",
    suit: "clubs",
    artist: "naniii",
...
const dump = async () => {
  ...
  const newCards = await Promise.all(
    cards.map(async (card) => {
      ...
      const deck = await Deck.findOne({ slug: card.deck });

      if (!deck) {
        // throw new Error(`Cannot reference deck: ${card.deck}.`);
        return;
      }
      ...
      if (card.artist) {
        ...
        if (!_id) {
          throw new Error(`Cannot reference artist: ${card.artist} in losers.`);
        }

        artist = _id;
      }

      return { ...card, artist, deck: deck._id };
    })
  );

  await Loser.insertMany(newCards);
};
